...
from modulos_modelos import (
    segmentacion_clientes, scoring_leads, prediccion_churn,
    panel_rendimiento_agentes, pricing_dinamico, marketing_personalizado,
    fidelizacion_clientes, analisis_sentimiento_nps, analizar_siniestros,
    priorizar_clientes_dormidos, digitalizar_renovaciones, calcular_panel_kpis
)
...
def segmentacion():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('segmentacion.html', grafico=grafico_path, descargar_csv=output_csv)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def scoring_leads_view():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('scoring_leads.html', descargar_csv=output_csv, descargar_xlsx=output_xlsx, reporte=reporte)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def churn_view():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('churn.html', reporte_csv=reporte_csv, reporte_xlsx=reporte_xlsx, predicciones_csv=predicciones_csv, reporte=reporte_dict)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def panel_agentes_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('panel_agentes.html', panel_csv=panel_csv, resumen_html=resumen_html)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def pricing_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('pricing.html', resultados_html=resultados_html, sim_html=sim_html, csv_path=csv_path, graf_path=graf_path)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def marketing_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('marketing.html', mensajes_html=mensajes_html, output_path=output_path)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def fidelizacion_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('fidelizacion.html', tabla_html=tabla_html, output_path=output_path)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def sentimiento_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('sentimiento.html', tabla_html=tabla_html, nps_score=nps_score, csv_out=csv_out)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def siniestros_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('siniestros.html', df_resultado=df_resultado.head(10).to_html(classes="table table-striped", index=False), cuellos=cuellos.to_html(classes="table table-striped", index=True), recomendaciones=recomendaciones)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def clientes_dormidos_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('clientes_dormidos.html', resultado=resultado.head(10).to_html(classes="table table-striped", index=False), file_export=file_export)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def renovaciones_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('renovaciones.html', resultado=resultado.head(10).to_html(classes="table table-striped", index=False), file_export=file_export)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def panel_kpis_view():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('panel_kpis.html', resultado=resultado.to_html(classes="table table-striped", index=False), file_export=file_export)
        except Exception as e:
            flash(resultado_mensaje(f"Error: {e}", exito=False), "danger")
...
def segmentacion_ndvi():
    ...
    if request.method == 'POST':
        ...
        if archivo_ndvi and archivo_parcela:
            ...
            try:
                mapa_html, recomendaciones, geojson, csv = segmentar_parcela_ndvi(ruta_ndvi, ruta_parcela, n_clusters)
                flash("Segmentación realizada con éxito.", "success")
            except Exception as e:
                flash(f"Error: {e}", "danger")
        else:
            flash("Debes subir ambos archivos (NDVI y GeoJSON).", "danger")
    return render_template('segmentacion_ndvi.html', mapa=mapa_html, recomendaciones=recomendaciones, geojson=geojson, csv=csv)
...
def deteccion_plagas():
    ...
    if request.method == 'POST':
        if 'labels_csv' in request.files and request.files['labels_csv']:
            ...
            carpeta_imgs = request.form.get('carpeta_imgs', 'imagenes_dron')
...
def estres_hidrico():
    ...
    if request.method == 'POST':
        ...
        resultado = detectar_estres_hidrico(ruta_termica, ruta_parcelas, limite_critico, cwsi_critico)
